using System.Security.Cryptography.X509Certificates;

using Fido2NetLib;

namespace Test;

public class CryptoUtilsTests
{
    [Fact]
    public void TestCertInCRLFalseCase()
    {
        byte[] certBytes = Convert.FromBase64String("");

        byte[] crl = Convert.FromBase64String("");

        var cert = X509CertificateLoader.LoadCertificate(certBytes);

        Assert.False(CryptoUtils.IsCertInCRL(crl, cert));
    }

    [Fact]
    public void TestCertInCRLTrueCase()
    {
        byte[] certBytes = Convert.FromBase64String("MIIDAjCCAqigAwIBAgIPBIBnSd4we71RdaJtc /  + 59Ah1hNfL5OEtxdgy +/ kIJyiScl4 + T8o4IBBTCCAQEwCwYDVR0PBAQDAgbAMAwGA1UdEwEB / wQCMAAwHQYDVR0OBBYEFPl4RxJ2M8prAEvqnSFK4 +  + 5VPY5gDcPXAdwA9Pudwe1M0BcGsa5 + gIhAJ0opi4Y / w26gNaAvsCvalwCqI6QYQCP1bjGSMgu3K1e");

        byte[] crl = Convert.FromBase64String("");

        var cert = X509CertificateLoader.LoadCertificate(certBytes);

        Assert.True(CryptoUtils.IsCertInCRL(crl, cert));
    }

    [Fact]
    public void TestValidateTrustChainRootAnchor()
    {
        var attestationRootCertificates = new X509Certificate2[3]
        {
            X509CertificateLoader.LoadCertificate(Convert.FromBase64String("")),
            X509CertificateLoader.LoadCertificate(Convert.FromBase64String("")),
            X509CertificateLoader.LoadCertificate(Convert.FromBase64String(""))
        };

        var trustPath = new X509Certificate2[2]
        {
            X509CertificateLoader.LoadCertificate(Convert.FromBase64String("")),
            X509CertificateLoader.LoadCertificate(Convert.FromBase64String(""))
        };
        Assert.True(CryptoUtils.ValidateTrustChain(trustPath, attestationRootCertificates).Result);
        Assert.False(CryptoUtils.ValidateTrustChain(trustPath, trustPath).Result);
        Assert.False(CryptoUtils.ValidateTrustChain(attestationRootCertificates, attestationRootCertificates).Result);
        Assert.False(CryptoUtils.ValidateTrustChain(attestationRootCertificates, trustPath).Result);
    }

    [Fact]
    public void TestValidateTrustChainSubAnchor()
    {
        // TODO: Figure out why this test fails on macOS and Linux
        if (!OperatingSystem.IsWindows())
            return;

        byte[] attRootCertBytes = Convert.FromBase64String("");
        var attestationRootCertificates = new X509Certificate2[1] { X509CertificateLoader.LoadCertificate(attRootCertBytes) };

        byte[] attCert = Convert.FromBase64String("");
        var trustPath = new X509Certificate2[1] { X509CertificateLoader.LoadCertificate(attCert) };

        Assert.False(0 == attestationRootCertificates[0].Issuer.CompareTo(attestationRootCertificates[0].Subject));
        Assert.True(CryptoUtils.ValidateTrustChain(trustPath, attestationRootCertificates).Result);
        Assert.True(CryptoUtils.ValidateTrustChain(trustPath, trustPath).Result);
        Assert.True(CryptoUtils.ValidateTrustChain(attestationRootCertificates, attestationRootCertificates).Result);
        Assert.False(CryptoUtils.ValidateTrustChain(attestationRootCertificates, trustPath).Result);
    }

    [Fact]
    public void TestValidateTrustChainSelf()
    {
        byte[] certBytes = Convert.FromBase64String("");
        var certs = new X509Certificate2[1] { X509CertificateLoader.LoadCertificate(certBytes) };

        byte[] otherCertBytes = Convert.FromBase64String("");
        var otherCerts = new X509Certificate2[1] { X509CertificateLoader.LoadCertificate(otherCertBytes) };

        Assert.True(CryptoUtils.ValidateTrustChain(certs, certs).Result);
        Assert.False(CryptoUtils.ValidateTrustChain(certs, otherCerts).Result);
    }
}
